...
if (!fs.existsSync(input)) {
  throw new Error("Run npm.cmd run robustness-experiments first.");
}
...
const rankValues = models.map(model => ({
  label: labels[model],
  value: Number((rows.filter(r => r.model === model).reduce((s, r) => s + r.rankShift, 0) / 5).toFixed(2))
}));

const allocationValues = models.map(model => ({
  label: labels[model],
  value: rows.filter(r => r.model === model).reduce((s, r) => s + r.allocationChanges, 0)
}));

const patrolValues = models.map(model => ({
  label: labels[model],
  value: Number((rows.filter(r => r.model === model).reduce((s, r) => s + r.patrolKm, 0) / 5).toFixed(2))
}));

fs.writeFileSync(path.join(outDir, "risk-model-comparison.svg"), barChart(
  "Canonical risk-model comparison",
  "Normalized maximum score by model; rankings are reported in research/RESULTS.md.",
  canonicalScores.map(([model, value]) => ({ label: labels[model], value })),
  100
));

fs.writeFileSync(path.join(outDir, "allocation-comparison.svg"), barChart(
  "Allocation sensitivity across five seeds",
  "Total count of zone-allocation changes relative to the frequency-only reference.",
  allocationValues,
